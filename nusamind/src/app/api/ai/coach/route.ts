...
export async function POST(req: Request) {
  try {
    ...
    const response = await callGroq(SYSTEM_PROMPTS.coach, JSON.stringify({ message: body.message, history: body.history }))
    const data = await parseJsonResponse<{ reply: string; suggestions: string[] }>(response)
