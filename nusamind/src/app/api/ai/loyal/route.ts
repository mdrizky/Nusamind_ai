...
export async function POST(req: Request) {
  try {
    ...
    const response = await callGroq(SYSTEM_PROMPTS.loyal, JSON.stringify(body))
    const data = await parseJsonResponse<{ follow_up_message: string; subject: string; segment_note: string; next_action: string }>(response)
