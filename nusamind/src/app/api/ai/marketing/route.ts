...
export async function POST(req: Request) {
  try {
    ...
    const response = await callGroq(SYSTEM_PROMPTS.marketing, JSON.stringify(body))
    const data = await parseJsonResponse<{ caption: string; hashtags: string[]; cta: string; platform_tips: string }>(response)
