...
export async function POST(req: Request) {
  try {
    ...
    const response = await callGroq(SYSTEM_PROMPTS.global, JSON.stringify(body))
    const data = await parseJsonResponse<{ translated_name: string; translated_description: string; export_tips: string[] }>(response)
