...
export async function POST(req: Request) {
  try {
    ...
    const response = await callGroq(SYSTEM_PROMPTS.reply, JSON.stringify(body))
    const data = await parseJsonResponse<{ reply: string; intent: string; tone: string }>(response)
