...
export async function POST(req: Request) {
  try {
    ...
    const response = await callGroq(SYSTEM_PROMPTS.campaign, JSON.stringify(body))
    const data = await parseJsonResponse<{ campaign_name: string; caption: string; broadcast_message: string; tips: string[]; hashtags: string[] }>(response)
