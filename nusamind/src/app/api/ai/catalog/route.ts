...
export async function POST(req: Request) {
  try {
    ...
    const response = await callGroq(SYSTEM_PROMPTS.catalog, JSON.stringify(body))
    const data = await parseJsonResponse<{ optimized_name: string; optimized_description: string; keywords: string[] }>(response)
